...
import { DATA, REZDY_MAP, SHIP } from "./_exp-data.mjs"; // generated by scripts/build-experiences.mjs
...
/* ship days, the same arithmetic as the page script: a cruise guest is planned back at the pier by the site's backBy (3:30pm) or an hour
   before the all-aboard time they gave; the one-way drive from their port decides whether the tour books on the spot at all.
   Returns an error string when this booking should not go through checkout, else null. */
const clockMins = (label) => { const t = timeToLocal(label); if (!t) return null; const [h, m] = t.split(":").map(Number); return h * 60 + m; };
export function shipDayProblem(venue, product, timeLabel, portSlug, aboardMins) {
  if (!SHIP || !portSlug) return null;
  const S = SHIP.settings, port = SHIP.ports[portSlug]; if (!S || !port) return null;
  const drive = venue.drive ? venue.drive[port.region] : null;
  if (SHIP.venues[venue.slug] === 0) return null; // airport lounges are flight-day things, a remembered port means nothing here
  if (drive != null && drive > S.askDrive) return `Not on a ship day from ${port.name}: too far for the hours in port. Ask us on WhatsApp and we'll suggest what fits.`;
  if (drive != null && drive > S.okDrive) return `A long day from ${port.name}. Send it as a WhatsApp request and we check it against your all-aboard time first.`;
  const sd = SHIP.products[`${venue.slug}:${product.id}`] || {};
  const backBy = Number(aboardMins) > 0 ? Number(aboardMins) - 60 : S.backBy;
  const stay = sd.flex ? Math.min(sd.mins || 120, 120) : (sd.mins || 180), d = drive || 0;
  const start = timeLabel ? clockMins(timeLabel) : (sd.starts && sd.starts.length ? sd.starts[0] : null);
  const fits = sd.no ? false : start != null ? start + stay + d <= backBy : S.arrive + d + stay + d <= backBy;
  if (!fits) return `That ${timeLabel ? "start" : "option"} can't get you back to the pier in time from ${port.name}. Pick an earlier start, or ask us on WhatsApp.`;
  return null;
}
export const aboardLabel = (mins) => { const m = Number(mins); if (!m) return ""; const h = Math.floor(m / 60) % 24, mi = m % 60; return `${h % 12 || 12}:${mi < 10 ? "0" : ""}${mi}${h < 12 ? "am" : "pm"}`; };
...
export function bookingSummary(meta) {
  return {
    ...
    cruise: meta.port_name || meta.ship ? [meta.ship, meta.port_name ? `from ${meta.port_name}` : "", meta.aboard ? `all-aboard ${aboardLabel(meta.aboard)}` : ""].filter(Boolean).join(", ") : "",
  };
}
