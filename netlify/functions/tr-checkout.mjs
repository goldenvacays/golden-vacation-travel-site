...
/* the offers a search would have shown: the hotel's own row, else the zone fallback */
export function offersFor(hotelSlug, zoneKey, airport, trip) {
...
export const handler = async (event) => {
  ...
  if (trip.key === "hotel") return json(400, { error: "Hotel to hotel rides are priced in the chat. Send it as a WhatsApp request." });
  ...
  const offers = offersFor(hotelSlug, zone.key, airport.code, trip.key);
  ...
  if (!ISO.test(date) || date < today) return json(400, { error: needIn ? "Pick your arrival date." : "Pick your departure date." });
  ...
  if (!place || (placeKind === "zone" && !/\(/.test(place))) return json(400, { error: "Where are you staying? The driver needs the hotel or the villa name." });
  const c = b.customer || {};
  ...
  const vehLabel = TR.vehicles[vehicle].label;
  const routeName = trip.key === "out" ? `${zone.name} to ${airport.name} airport` : trip.key === "both" ? `${airport.name} airport to ${zone.name}, round trip` : `${airport.name} airport to ${zone.name}`;
  const productName = `${routeName}, ${vehLabel.toLowerCase()} up to ${seats}, ${trip.label.toLowerCase()}`;
  const whenDesc = [flightIn ? `arrives ${longDate(date)} on ${flightIn} at ${timeIn}` : "", flightOut ? `departs ${longDate(trip.key === "both" ? date2 : date)} on ${flightOut} at ${timeOut}` : ""].filter(Boolean).join(", ");
  const desc = `${productName}. ${whenDesc}. ${people} ${people === 1 ? "person" : "people"}, ${place}.`;
  /* the metadata keys match the Experiences bookings so exp-webhook, exp-status and the team's forms read it the same way */
  const metadata = {
    ref, kind: "transfer", slug: hotelSlug || zone.key, product: `${vehicle}-${seats}-${trip.key}`, venue_name: "Airport transfer", product_name: productName, confirm: "team",
    date, time: "", date2, flight_in: flightIn, flight_out: flightOut, rezdy_code: "", rezdy_start: "",
    adults: String(people), children: "0", pickup: placeKind, pickup_label: place, pickup_hotel: "", choices: "",
    ship: "", port: "", port_name: "", aboard: "", first, last, email, phone, total_usd: String(total), rezdy_status: "", note: [note, timeIn ? `lands ${timeIn}` : "", timeOut ? `takes off ${timeOut}` : ""].filter(Boolean).join(" · "), airport: airport.code, zone: zone.key, seats: String(seats),
  };
  try {
    const session = await stripe("/checkout/sessions", {
      ...
      line_items: [{ quantity: 1, price_data: { currency: "usd", unit_amount: Math.round(total * 100), product_data: { name: `Airport transfer: ${productName}`, description: desc.slice(0, 500) } } }],
      metadata,
      payment_intent_data: { metadata, description: `${ref} Airport transfer: ${desc}`.slice(0, 1000) },
